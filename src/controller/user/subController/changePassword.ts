...
const changePassword = async (req: Request, res: Response) =>{
    ...
    try {
        ...
        if(!isMatched)return res.status(400).send({success: 'false', message: "old password didn't matched"}) 

        if(newPassword !== confirmPassword) return res.status(400).send({success: 'false', message: "newPassword and confirmPassword didn't matched"})
